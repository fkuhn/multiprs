...
def align_tier_events(ttl):
    ...
    for tupel in ttl:
        ...
        try:
            ...
            print "error finding startevent"
            print "fehler: " + it_v[0].text + ' ' + it_pos[0].text
...
def make_input_file(annofiledir, validtagsfilename, traininputfilename):
    ...
    print "Reading directory " + annofiledir + "\n"
    ...
    for tupellist in output_list:
        ...
        for tupel in tupellist:
            ...
            if not (not tupel[0] or not (tupel[1])):
                #check if pos tag is valid and write to outputfile
                #else do not write anything

                try:
                    if vtags.is_valid_postag(tupel[1]):

                        #trainingfile.write(tupel[0] + '\t' + vtags.get_nearest_tag(tupel[1]) + '\n')
                        pos = tupel[1]
                        pos = vtags.del_remarks(pos)
                        trainingfile.write(tupel[0] + '\t' + pos + '\n')

                    else:
                        if vtags.is_valid_postag_composite(tupel[1]):
                            #trainingfile.write(tupel[0] + '\t' + vtags.get_nearest_tag(tupel[1]) + '\n')
                            trainingfile.write(tupel[0] + '\t' + tupel[1] + '\n')
                except:
                    print tupel[1]
            tokenposnumber += 1
    ...
    print "script terminated with " + str(filenumber) + " files processed and " + str(
