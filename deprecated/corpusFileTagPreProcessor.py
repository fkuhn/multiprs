...
class ValidTags:
    ...
    def __init__(self, tagfilename, remarks):
        ...
        self.tags = codecs.open(tagfilename, encoding='utf-8').read().rsplit('\n')
    ...
    def is_valid_postag(self, tagstring):
        ...
        """
        try:
            #tagmatch = re.search('(^[A-Z]+)([1-3]?)([A-Z]*)', tagstring)
            tagmatch = re.match('(^[A-Z]+)([1-3]?)([A-Z]*)', tagstring, re.UNICODE)
    ...
    def del_remarks(self, tagstring):
        ...
        """
        t = tagstring
        tagmatch = re.match('([A-Z]+)([1-3]?)([A-Z]*)', t, re.UNICODE)
        if tagmatch.group(3):
        ...
        for item in tagstring:
            if len(tagstring) > 1:
                ...
                if self.is_valid_postag(tagstring):
                    print tagstring
                    composite_count += 1
                    #if valid substring is found. recursevily invoke this method wi th the rest of the string
